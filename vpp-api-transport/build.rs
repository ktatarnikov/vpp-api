...
fn main() {
    ...
    let workspace_dir = workspace_dir_path.to_str().expect("CARGO_WORKSPACE_DIR is invalid");

    let arch = std::env::var("CARGO_CFG_TARGET_ARCH").expect("architecture is not defined with CARGO_CFG_TARGET_ARCH variable");
    ...
    let value: toml::Value = toml::from_str(&cargo_toml)
        .expect("invalid Cargo.toml format");
    ...
    if !std::path::Path::new(&format!("{}/{}", &vpp_lib_dir, &library_filename)).exists() {
        panic!("Can not find libvppapiclient.so.<version> at {}", vpp_lib_dir)
    };

    let flags = format!("cargo:rustc-flags=-L\"{}\",-l:{}", &vpp_lib_dir, &library_filename);
    ...
    loop {
        ...
        if manifest.exists() {
            let text = fs::read_to_string(&manifest)
                .expect("failed reading Cargo.toml");
